...
class CoinStore:
    ...
    async def rollback_to_block(self, block_index: int) -> List[CoinRecord]:
        ...
        pgdb_c1 = self.pgdb.cursor()
        pgdb_c1.execute("DELETE FROM coin_record WHERE confirmed_index>%s", (block_index,))
        self.pgdb.commit()
        pgdb_c1.close()
        ...
        pgdb_c2 = self.pgdb.cursor()
        pgdb_c2.execute("UPDATE coin_record SET spent_index = 0, spent = 0 WHERE spent_index>%s", (block_index,))
        self.pgdb.commit()
        pgdb_c2.close()
    ...
    async def _add_coin_records(self, records: List[CoinRecord]) -> None:
        ...
        pgdb_cursor.executemany(
            "INSERT INTO coin_record (coin_name, confirmed_index, spent_index, spent, coinbase, puzzle_hash, address, coin_parent, amount, timestamp) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
            pgdb_values,
        )
        self.pgdb.commit()
        pgdb_cursor.close()
